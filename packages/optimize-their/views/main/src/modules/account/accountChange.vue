<template>
  <div>
    <div class="modal-btn" @click="show">创建新账户</div>
    <div class="form-box" v-if="modalSwicth" @click.stop="close">
      <div class="form-main" @click.stop="show">
        ...
        />
        <select class="form-select" name="" id="" v-model="formData.type">
...
export default {
  ...
  setup() {
    ...
    const config = ref([]);
    const formData = reactive({name: "",type: "借记卡"});
    const modalSwicth = ref(false);
    function getPageData(data) {
      electron.ipcRenderer
        .invoke("accountChange", JSON.stringify(data))
        .then(result => {
          console.info(result);
          close();
        });
    ...
    function getPageDatasel() {
      electron.ipcRenderer.invoke("getAccountType").then(result => {
        config.value = result;
      });
    }
    function updateData() {
